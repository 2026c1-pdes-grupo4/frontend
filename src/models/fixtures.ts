import type { User, Agency, AgencyProperty, Favorite, Purchase, TopBuyer, TopRankedProperty, TopAgencySales } from './types'
...
type AgencyPropertyListing = Pick<AgencyProperty,
  'id' | 'propertyId' | 'address' | 'city' | 'propertyType' | 'listedPrice' | 'listedDate' | 'available' | 'agencyId' | 'agencyName'>

const agencyPropertyListings: AgencyPropertyListing[] = [
...
export const agencyProperties: AgencyProperty[] = agencyPropertyListings.map((listing) => {
  const base = properties.find((p) => p.propertyId === listing.propertyId)
  return {
    ...listing,
    province: base?.province ?? '',
    areaSq: base?.areaSq ?? 0,
    rooms: base?.rooms ?? 0,
